...
class TargetModel:
    ...
    def construct_model(self, model_name, dataset_shortname, w, h):
        if model_name == 'inception':
            ...
            self.model = ResNet50(weights=dataset_shortname)
            from keras.applications.resnet50 import preprocess_input, decode_predictions
            self.preprocess_input = preprocess_input
            self.decode_predictions = decode_predictions
        elif model_name == 'vgg':
            self.model = VGG16(weights=dataset_shortname)
    ...
    def evaluate_model(self, img_path):
        imgs = []
        img = image.load_img(img_path, target_size=self.target_size) #(299, 299))
        ...
        while 1:
            for i in range(0, len(self.val_label), self.batch_size):
                ...
                for img_name in img_list:
                    img = image.load_img(os.path.join(path, img_name), target_size=self.target_size) #(299, 299))
